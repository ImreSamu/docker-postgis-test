...
fetch_remote_state() {
    ...
    log_info "postgis_all_v3_versions_array_string ="
    echo "$postgis_all_v3_versions_array_string" \
        | tr ' ' '\n' \
        | sed '/^$/d' \
        | awk -F. '{
            key = $1 "." $2
...
generate_debian() {
    ...
    if [[ -z "$postgisFullVersion" ]]; then
		...
		# =================================================================
		# PLACEHOLDER DOCKERFILE - Build not available
		# =================================================================
		# Package:    ${postgisPackageName:-unknown}
		# Suite:      ${debian_suite}
		# PostgreSQL: ${postgres_main}
		# PostGIS:    ${postgis_version}
		# Reason:     Debian package not available for this combination
